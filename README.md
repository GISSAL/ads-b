...
| Flight Duration Threshold (secs) | Enter a duration threshold (in seconds) that defines the minimum time between successive aircraft waypoints that must pass before a new flight by that aircraft is considered to occur.  The default value is 900 secs (15 mins).      | Required | Input     | Long      |
